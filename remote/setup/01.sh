...
# Prompt to enter a password for the PostgreSQL kcsda user (rather than hard-coding
# a password in this script).
read -p "Enter password for kcsda DB user: " DB_PASSWORD
...
# Set up the cas DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE casdb"
sudo -i -u postgres psql -d casdb -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d casdb -c "CREATE ROLE kscda WITH LOGIN PASSWORD '${DB_PASSWORD}'"
# Add a DSN for connecting to the greenlight database to the system-wide environment
# variables in the /etc/environment file.
echo "DATABASE_DSN='postgres://greenlight:${DB_PASSWORD}@localhost/greenlight'" >> /etc/environment
